...
Dodatkowo, w uzgodnieniu z użytkownikami aplikacji przygotowane zostanie
...
uruchomiona pełna funkcjonalność aplikacji. Przypadki testowe będą zawierały m.
in. przypadki dodawania nowych samochodów, dodawania użytkowników,
...
Algorytm ten wykorzystuje metode `checkIfPeriodsOverlap`, która sprawdza czy dwa okresy się pokrwają (okres 'TimePeriod' jest obiektem zawierającym dwie daty
- datę "od" oraz datę "do").
Poniżej zamieszczono kod testu jednostkowego spradzającą poprawność działania `checkIfPeriodsOverlap`:
```groovy
    @Unroll
...
```
Aby uruchomić test konieczne było utworzenie mocków dla wykorzystanych obiektów:
```groovy
...
Walidator stanowi odrębną klase, z główna metodą `isValid`, która używa odpowiedniej logiki do zweryfikowania poprawności wprowadzonego okresu czasu (TimePeriod).
...
Metoda testowa wykorzystuje zamockowane obiekty:
```groovy
...
### 16.1.1. Wdrożenie z wykorzysaniem pliku jar
